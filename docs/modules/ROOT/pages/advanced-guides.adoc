...
Combined with the <<bundle-tag>>, this file will be rendered with the scripts and styles tags to include in your html template The <<bundle-import-map>> tag is also available. This file will be served as the default index file (e.g. http://localhost:8080/).
...
[#extension-web-dependencies]
=== Web Dependencies provided by Quarkus extensions

Some Quarkus extensions ship JavaScript modules for the browser (for example the https://github.com/quarkiverse/quarkus-json-rpc[JSON-RPC] extension client). They are not `org.mvnpm` or `org.webjars` dependencies, so they declare them by producing a `WebDependencyJarBuildItem` (from `quarkus-vertx-http-deployment-spi`, Quarkus 3.35.4+). The Web Bundler picks them up the same way as the https://quarkus.io/guides/web-dependency-locator[Web Dependency Locator]; you don't have to configure anything:

- If the extension provides import mappings (e.g. `@quarkiverse/json-rpc` -> `/_static/quarkus-json-rpc/jsonrpc-client.js`), the modules are served by the extension. They are excluded from the bundling (marked as external) and added to the <<bundle-import-map>>, so the browser resolves them at runtime. Prefix mappings (ending with `/`) exclude everything under them.
- Otherwise, the extension JAR follows the mvnpm layout (with a `package.json`) and it is installed in the <<node_modules>> to be bundled like any other <<mvnpm>> dependency.

For modules served by an extension, add the `{#bundleImportMap /}` tag to your page (in `src/main/resources/web/` or in a Qute template), *before* the `{#bundle /}` tag:
...
Then import them from your scripts as usual:
...
==== Providing web modules from your extension

Extension authors declare their web modules in a deployment `@BuildStep` (add the `io.quarkus:quarkus-vertx-http-deployment-spi` dependency to the deployment module):

[source,java]
----
@BuildStep
void webDependencies(CurateOutcomeBuildItem curateOutcome, BuildProducer<WebDependencyJarBuildItem> webDependencies) {
    curateOutcome.getApplicationModel().getDependencies().stream()
            .filter(dep -> dep.getGroupId().equals("org.acme") && dep.getArtifactId().equals("acme-greeter"))
            .findFirst()
            .ifPresent(dep -> webDependencies.produce(new WebDependencyJarBuildItem(
                    dep.getKey(),
...
----
<1> The import mappings to the modules your extension serves (e.g. from `META-INF/resources/_static/` or a route). Use the two arguments constructor instead if the JAR follows the mvnpm layout, it will then be bundled.

The same build item is used by the https://quarkus.io/guides/web-dependency-locator[Web Dependency Locator], so both extensions support your modules.
...
It also contains the import mappings of the modules served by Quarkus extensions (see <<extension-web-dependencies>>), which is required for your bundle to load them.

NOTE: Place `{#bundleImportMap /}` before `{#bundle /}` and any other module script, browsers only apply an import map to modules loaded after it.
